...
- 데이터베이스 모델 반영
  - 터미널 실행  ``python manage.py makemigrations``
...
- 데이터베이스 모델 적용
  - 터미널 실행 ``python manage.py migrate boards``
...
- Django Extentions를 설치하면 shell_plus 사용이 가능하다.
  - 터미널에서 ``pip install django-extensions``
...
- Django에 있는 모델들을 모두 import 된 상태로 사용 가능하게 한다.
  - 터미널에서 ``python manage.py shell_plus``
...
- shell_plus를 사용하면 아래와 같이 import 할 필요 없이 바로 사용이 가능하다.
...
- 이제 관리자 페이지에서 댓글들을 확인 할 수 있도록 설정 해줘야 한다.
  - boards - admin.py 코드 수정
...
- Comment의 URL path 설정 및 변수명 조금 더 명시적으로 표기( id -> board_id )
