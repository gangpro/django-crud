...
* boards - migrations 폴더는 데이터베이스의 설계도 같은 곳이다. 

* 반드시!! model 작성 끝나면 아래와 같이 처음에 설정을 해줘야 한다.

* boards - models.py에서 데이터베이스 이렇게 이렇게 사용하겠다 작성했으니 이제 데이터베이스에 저장할 수 잇도록 설계도 같은 곳이 boards-migrations이다. 시작하기 앞서 아래와 같이 꼭 해야함

* 터미널 실행 후 아래와 같이 코드 작성
...
* 반면, crud-settings.py에서 INSTALLED_APPS 설정안하면 아래와 같이 뜨면 안된다.
...
* boards - models.py 를 수정하는 경우가 생기면 python manage.py makemigrations를 실행해서 새롭게 설계도를 해야한다.
